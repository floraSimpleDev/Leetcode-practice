...
Design an `EventEmitter` class. This interface is similar (with some differences) to the one found in Node.js or the Event Target interface of the DOM.
...
- the name of an event as a string
- a callback function

This callback function will later be called when the event is emitted.

An event can have multiple listeners for the same event.

When emitting an event with multiple callbacks:

- each callback should be called in the order in which it was subscribed
...
- the name of an event as a string
- an optional array of arguments that will be passed to the callback(s)
...
The emit method should be able to accept an optional array of arguments.
...
All test cases are valid, so you do not need to handle unsubscribing from a non-existing subscription
...
The EventEmitter action does not take any arguments

The emit action takes either 1 or 2 arguments:

the event name

an optional array of arguments passed to the callback functions

The subscribe action takes 2 arguments:
