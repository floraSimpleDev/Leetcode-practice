...
The array has been split into subarrays with **3 elements**.  
Only **two elements remain** for the last subarray.
...
`size` is greater than `arr.length`, so all elements remain in **one subarray**.
