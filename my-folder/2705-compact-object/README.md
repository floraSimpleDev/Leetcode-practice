...
A compact object is the same as the original object, except that **keys containing falsy values are removed**. This operation applies to the object and any nested objects.

Arrays are considered objects where the indices are keys.

A value is considered **falsy** when `Boolean(value)` returns `false`.

You may assume `obj` is the output of `JSON.parse`. In other words, it is **valid JSON**.
